...
"""
    ...
    for user_id, username in success_logs.items():
        event_viewer_text.insert(tk.END, f"User ID: {user_id}, User Name : {username}\n")
    event_viewer_text.insert(tk.END, "\nError Logs per User ID:\n")
    for user_id, username in error_logs.items():
        event_viewer_text.insert(tk.END, f"User ID: {user_id}, User Name: {username}\n")
